...
class Render(surfaceTexture: SurfaceTexture): IRenderListener {
    ...
    companion object {
        ...
        // 修改片段着色器以支持普通 MP4
        private const val FRAGMENT_SHADER_NORMAL_MP4 = "#extension GL_OES_EGL_image_external : require\n" +
...
//        shaderProgram = ShaderUtil.createProgram(RenderConstant.VERTEX_SHADER, RenderConstant.FRAGMENT_SHADER)
        shaderProgram = ShaderUtil.createProgram(RenderConstant.VERTEX_SHADER, FRAGMENT_SHADER_NORMAL_MP4)
    ...
    override fun setAnimConfig(config: AnimConfig) {
        setVertexBuf(config)
        setTexCoords(config)
        hasAlpha = config.hasAlpha  // 保存是否有 alpha 通道
    }
...
// 设置是否有 alpha 通道
        GLES20.glUniform1i(uHasAlphaLocation, if (hasAlpha) 1 else 0)
